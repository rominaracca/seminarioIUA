...
public class ProductsRSControllerTest {
	...
	public void updateProductNotFound() throws Exception {
		...
		mockMvc.perform(put(Constants.URL_PRODUCTS + "/-999").contentType(MediaType.APPLICATION_JSON)
	...
	public void performCicle() throws Exception {
		...
		p.setPrice(150.50);
		p.setCode("CODE_TEST_UPDATE");
		Category c_up = new Category();
		c_up.setDescription("Categoria Test Up");
		p.setCategory(c_up);
		p.getTags().add("test 4");
		p.getTags().remove(0);
		p.getTags().remove(0);
		mockMvc.perform(put(Constants.URL_PRODUCTS + "/" + id).contentType(MediaType.APPLICATION_JSON)
				.content(TestUtil.convertObjectToJsonBytes(p)).accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk()).andExpect(content().contentType("application/json;charset=UTF-8"))
				.andExpect(jsonPath("$.description", is(p.getDescription())))
				.andExpect(jsonPath("$.price", is(p.getPrice())))
